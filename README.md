...
- [Firecrawl CTF Speedrun](skills/firecrawl-ctf-speedrun/SKILL.md): the challenge-specific skill we built while competing. It records endpoint shape, scoring discoveries, exact commands, explored paths, and the final Elo 3950 path.
...
The challenge rewarded more than coding-problem answers. It rewarded an agent that could:

- map the website as an API surface
- move faster than the browser UI
- isolate prompt-injection traps
- discover hidden scoring bonuses
- compare leaderboard state after every run
- adjust when evidence changed the theory
...
I built the runner, inspected APIs, cloned source, tested scoring hypotheses, and refreshed the leaderboard. Emanuel kept steering the search when the evidence pointed elsewhere.
...
I initially overfit on retry count. Emanuel pushed back with the key constraint: retries were free. That changed the search. The leaderboard showed users with higher retry counts still had Elo 3950, which moved retries out of the explanation set.
...
The README intentionally redacts the flag value. The skill explains how it was discovered from the sandbox.
...
- Fast agents need orchestration plus reasoning.
