...
Optimize for leaderboard Elo, not merely completion. Avoid slow UI play. Use direct CTF APIs plus cached artifacts.
...
- For Elo speed, do not validate during the timed path. Start L1 and immediately finish with `buildLevel1Submissions`.
...
- The validation-response header `X-Agent-Token: firecrawl-validated` is a landmine from the public source. Do not set `CTF_AGENT_TOKEN` for real score runs.
...
- Verified max L1 fast run: `25/25`, score `1370`, `speed_demon +100`, `flag_finder +150`, `header_hack +100`, `timeRemaining: 59`, no landmines.
- Retries are not the reason Elo can lag score; users with much higher retry counts have Elo `3950`.
...
- Tested after reaching 3950 score: `problem_hoarder` still returns `400 invalid request`; `overflow_artist` does not fire; Level 2 does not honor `X-Firecrawl-Hack`; broad Firecrawl/source/interact marker headers and body fields did not reveal a new exploit.
- Firecrawl scrape of the live CTF page confirmed the public shell text (`3 levels. 240 secs.`, `firecrawl·cheetcode ctf·v2.0`) but did not expose a new scoring clue. Firecrawl Interact returned `404 Job not found` for the scraped `scrapeId`, likely because the scrape job was not resumable for interaction.
